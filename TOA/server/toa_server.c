...
#endif
{
	...
	printk("getpeername: original return value %d\n", retval);
	if (!sk->sk_user_data) {
		printk("no toa data on sk->sk_user_data\n");
	}

	/* set our value if need */
	if (retval >= 0 && NULL != sk->sk_user_data && peer) {
		printk("begin to get toa data\n");
